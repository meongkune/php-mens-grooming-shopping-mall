...
<?
		...
		$uid = $_REQUEST["uid"]; /* isset($_GET['uid']) ? $_GET['uid'] : ''; // GET으로 값 받기 */

		$sql = "SELECT * FROM member where uid='$uid'";
		$result = mysqli_query($db, $sql);
		if (!$result) exit("에러: $sql");

		$count = mysqli_num_rows($result); // 레코드 개수 사실상 0 아니면 1개인데...
?>
